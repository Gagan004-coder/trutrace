...
async function initDB() {
  ...
  try {
    ...
    `);
    // Add reset columns to existing tables if they don't exist yet
    await conn.execute(`ALTER TABLE users ADD COLUMN IF NOT EXISTS reset_token VARCHAR(10) DEFAULT NULL`).catch(() => {});
    await conn.execute(`ALTER TABLE users ADD COLUMN IF NOT EXISTS reset_expires DATETIME DEFAULT NULL`).catch(() => {});
